...
def _draw_self_assessment_watermark(canvas):
    """Diagonal semi-transparent watermark on every page."""
    canvas.saveState()
    canvas.setFillColor(colors.Color(0.72, 0.05, 0.05, 0.09))
    canvas.setFont("Helvetica-Bold", 52)
    canvas.translate(PAGE_W / 2, PAGE_H / 2)
    canvas.rotate(35)
    canvas.drawCentredString(0, 0.38 * inch, "SELF-ASSESSMENT ONLY")
    canvas.setFont("Helvetica-Bold", 26)
    canvas.drawCentredString(0, -0.42 * inch, "NOT A LEGAL CERTIFICATION")
    canvas.restoreState()
...
def _draw_detail_header(canvas, doc):
    ...
    _draw_self_assessment_watermark(canvas)
    canvas.saveState()
    w, h = PAGE_W, PAGE_H
...
def _build_page1(canvas, compliance: Dict, certificate_id: str):
    ...
    # ── SELF-ASSESSMENT ONLY banner ───────────────────────────────────────────
    banner_h = 0.24 * inch
    banner_w = inner_w * 0.88
    canvas.setFillColor(RED)
    canvas.roundRect(cx - banner_w / 2, y - banner_h, banner_w, banner_h, 4, fill=1, stroke=0)
    canvas.setFillColor(colors.white)
    canvas.setFont("Helvetica-Bold", 8.5)
    canvas.drawCentredString(cx, y - banner_h + 0.08 * inch,
        "SELF-ASSESSMENT ONLY  ·  NOT A LEGAL CERTIFICATION BY A NOTIFIED BODY")
    y -= banner_h + 0.18 * inch
...
def _build_page2_story(compliance: Dict, certificate_id: str, s: Dict):
    ...
    for key, article in articles.items():
        ...
        if status == "partial":
            detail_rows.append([Paragraph(
                '<font color="#92400e"><b>⚠ DECLARATION ONLY</b> — No supporting documentation '
                'attached. Attach evidence notes and date to upgrade to PASS.</font>',
                s["p2_evidence"],
            )])
        detail_tbl = Table(detail_rows, colWidths=[6.35 * inch])
    ...
    story.append(Paragraph(
        "SELF-ASSESSMENT ONLY — This report is generated by Pragma (pragma.ai) and constitutes an informational "
        "self-assessment, not a legal certification by a notified body. Articles marked PARTIAL represent "
        "declaration-only controls with no supporting documentation attached; they do not satisfy formal EU AI Act "
        "conformity requirements. High-risk AI systems require conformity assessment by a notified body prior to EU "
        "market placement. Renew this report annually or after any material system change. Not legal advice.",
        s["p2_meta"],
    ))
    return story
